...
NET_PROTO = "/dl/model/MobileNet-SSD/proto/union/MobileNetSSD_deploy_concat.prototxt"
...
for img_path in os.listdir(img_path_root):
    ...
    out_ssd = out["union_out"][:, 2*320*480:, :, :].reshape([1, 1, -1, 7])
    # np.reshape(out_ssd, (1, 1, -1, 7))

    box, conf, cls = postprocess(img_resize, out_ssd)

    out_seg = out["union_out"][:, 0:2*320*480, :,:].reshape([1, 2, 320, 480])
    # np.reshape(out_seg, (1, 2, 320, 480))
